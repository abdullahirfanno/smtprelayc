...
import (
	...
	"strings"
	"time"
	"github.com/abdullahirfanno/inssmtpd"
)
...
func connectionChecker(peer smtpd.Peer) error {
	...
	fmt.Println("Allowed Network")
	for i := range nets {
		_, allowedNet, _ := net.ParseCIDR(nets[i])

		if allowedNet.Contains(peerIP) {
			fmt.Println("Allowed Network Peer IP ")
			return nil
		}
	}
...
func authChecker(peer smtpd.Peer, username string, password string) error {
	fmt.Println("Auth Checker is running");
...
func mailHandler(peer smtpd.Peer, env smtpd.Envelope) error {
	fmt.Println("Received mail in request from");
	peerIP := ""
	if addr, ok := peer.Addr.(*net.TCPAddr); ok {
